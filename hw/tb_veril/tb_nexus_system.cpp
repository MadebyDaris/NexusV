...
int main(int argc, char** argv, char** env) {
    ...
    // Reset sequence exactly as in X-HEEP tb_top.cpp
    ...
    // Hold in reset, set BootROM bypass BEFORE releasing reset
    runCycles(20);

    // Set scope and call DPI function to bypass BootROM
    ...
    svSetScope(scope);
    tb_set_exit_loop();
    std::cout << "[TB] Triggered BootROM exit loop" << std::endl;
    ...
    while (!Verilated::gotFinish() && cycles < max_cycles) {
        ...
        if (top->clk_i) { // Only evaluate logic on posedge
            cycles++;
            if (top->exit_valid_o) {
                test_finished = true;
